    ...
    <div className="w-full space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {files.map((item) => (
          <div
            key={item.id}
            className="group relative rounded-xl overflow-hidden bg-gray-50 border border-[#EEDDD8] shadow-sm aspect-square flex flex-col justify-between"
          >
            {/* Image Preview */}
            <div className="relative w-full h-full">
              {item.previewUrl ? (
                ...
                <div className="w-full h-full flex items-center justify-center bg-gray-200 text-xs text-[#756A70]">
                  Önizleme yok
                </div>
              )}
              ...
              {!isUploading && item.status !== 'completed' && (
                <button
                  ...
                  className="absolute top-1.5 right-1.5 w-7 h-7 rounded-full bg-black/60 hover:bg-rose-600 text-white flex items-center justify-center transition-colors shadow-md backdrop-blur-sm"
            ...
            <div className="absolute bottom-0 inset-x-0 p-1.5 bg-gradient-to-t from-black/80 via-black/40 to-transparent text-white text-[11px] truncate flex justify-between items-center pointer-events-none">
